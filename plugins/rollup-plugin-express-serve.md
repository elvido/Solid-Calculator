...
| `proxy`              | `Record<string, string \| { target: string, stripPrefix?: boolean, rewrite?: string }>`                     | `undefined`   | Proxy configuration: route to target mapping. Supports prefix stripping or an explicit target path rewrite.                                                                           |
